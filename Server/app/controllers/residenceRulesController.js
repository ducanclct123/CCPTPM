...
const residenceRulesController = {
    ...
    addResidenceRule: async (req, res) => {
        try {
            ...
            res.status(201).json({
                message: "Residence rule added successfully",
                status: true,
            });
        } catch (err) {
            console.error(err);
    ...
    updateResidenceRule: async (req, res) => {
        try {
            ...
            res.status(200).json({
                message: "Residence rule updated successfully",
                status: true,
            });
        } catch (err) {
            console.error(err);
    ...
    deleteResidenceRule: async (req, res) => {
        try {
            ...
            res.status(200).json({
                message: "Residence rule deleted successfully",
                status: true,
            });
        } catch (err) {
            console.error(err);
    ...
    getResidenceRuleById: async (req, res) => {
        try {
            ...
            const [rule] = await db.execute(
                "SELECT * FROM residence_rules WHERE id = ?",
                [ruleId],
            );

            if (rule.length === 0) {
                res.status(404).json({
                    message: "Residence rule not found",
                    status: false,
                });
            } else {
    ...
    searchResidenceRules: async (req, res) => {
        try {
            ...
            const [rules] = await db.execute(queryStr, [
                searchQuery,
                searchQuery,
            ]);
