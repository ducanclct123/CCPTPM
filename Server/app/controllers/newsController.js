...
const newsController = {
    ...
    getNewsById: async (req, res) => {
        const { id } = req.params;
        try {
            const [singleNews] = await db.execute(
    ...
    createNews: async (req, res) => {
        ...
        try {
            await db.execute(
                "INSERT INTO news (name, description, image) VALUES (?, ?, ?)",
    ...
    updateNewsById: async (req, res) => {
        ...
        try {
            if (image) {
                ...
                await db.execute(
                    "UPDATE news SET name=?, description=?, image=? WHERE id=?",
                    [name, description, image, id],
                );
            } else {
                // Chỉ cập nhật name và description nếu image không được cung cấp
                await db.execute(
                    "UPDATE news SET name=?, description=? WHERE id=?",
                    [name, description, id],
                );
            }
    ...
    searchNews: async (req, res) => {
        ...
        try {
            const [news] = await db.execute(
                "SELECT * FROM news WHERE name LIKE ? OR description LIKE ?",
                [`%${query}%`, `%${query}%`],
            );
            res.json(news);
        } catch (error) {
