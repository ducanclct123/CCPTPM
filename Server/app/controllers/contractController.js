...
const contractController = {
    createContract: async (req, res) => {
        try {
            ...
            const query =
                "INSERT INTO contracts (vendor_id, title, start_date, end_date, description, value, file_url) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    updateContract: async (req, res) => {
        try {
            ...
            const query =
                "UPDATE contracts SET vendor_id = ?, title = ?, start_date = ?, end_date = ?, description = ?,file_url = ?, value = ? WHERE id = ?";
    ...
    searchContracts: async (req, res) => {
        try {
            ...
            if (keyword) {
                conditions.push(
                    "(contracts.title LIKE ? OR vendors.name LIKE ?)",
                );
                params.push(`%${keyword}%`);
                params.push(`%${keyword}%`);
            }
